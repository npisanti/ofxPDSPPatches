...
ofParameterGroup & ofx::patch::tuning::PureTuning::setup( int oscsNum, std::string name ) {
...
ofParameterGroup & ofx::patch::tuning::PureTuning::label( std::string name ){
    parameters.setName(name);
    return parameters;
}

void ofx::patch::tuning::PureTuning::updateAll( int & value ) {
    int dummy = 0;
    for( RatioUI & r : ratios ) r.ratioChange( dummy );
}

pdsp::Patchable & ofx::patch::tuning::PureTuning::out_pitch(int voice) {
    return ratios[voice].pitch;
}

void ofx::patch::tuning::PureTuning::smoothing( float ms ) {
    for( RatioUI & r : ratios ) r.pitch.enableSmoothing( ms );
}

float ofx::patch::tuning::PureTuning::pitch( int voice )const{
