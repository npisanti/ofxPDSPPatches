...
void ofx::patch::synth::MultiSampler::patch (){
...
ofParameterGroup & ofx::patch::synth::MultiSampler::label( std::string name ){
    parameters.setName( name );
    return parameters;
}

void ofx::patch::synth::MultiSampler::addSample( pdsp::SampleBuffer & sample) {
...
void ofx::patch::synth::MultiSampler::dBTriggering( bool enable ) {
    if(enable){
        env.enableDBTriggering(-24.0f, 0.0f);
    }else{
...
pdsp::Patchable & ofx::patch::synth::MultiSampler::in_trig() {
    return in("trig");
}

pdsp::Patchable & ofx::patch::synth::MultiSampler::in_select() {
    return in("select");
}

pdsp::Patchable & ofx::patch::synth::MultiSampler::in_pitch() {
    return in("pitch");
}

pdsp::Patchable & ofx::patch::synth::MultiSampler::out_0() {
    return out("0");
}
pdsp::Patchable & ofx::patch::synth::MultiSampler::out_1() {
    return out("1");
}
pdsp::Patchable & ofx::patch::synth::MultiSampler::out_L() {
    return out("0");
}
pdsp::Patchable & ofx::patch::synth::MultiSampler::out_R() {
    return out("1");
}
