...
void ofx::patch::synth::QuickSampler::patch(){
...
ofParameterGroup & ofx::patch::synth::QuickSampler::label( std::string name ){
    parameters.setName( name );
    return parameters;
}

void ofx::patch::synth::QuickSampler::loadButtonCall( bool & value ) {
    if(value){
...
void ofx::patch::synth::QuickSampler::sampleChangedCall( std::string & value ) {
...
void ofx::patch::synth::QuickSampler::loadSample( std::string path ) {
...
void ofx::patch::synth::QuickSampler::load( std::string path ) {
    samplePath = path;
}

void ofx::patch::synth::QuickSampler::dBTriggering( bool enable ) {
    if(enable){
        env.enableDBTriggering(-24.0f, 0.0f);
    }else{
...
pdsp::Patchable & ofx::patch::synth::QuickSampler::in_trig() {
    return in("trig");
}
pdsp::Patchable & ofx::patch::synth::QuickSampler::in_pitch() {
    return in("pitch");
}

pdsp::Patchable & ofx::patch::synth::QuickSampler::out_0() {
    return out("0");
}
pdsp::Patchable & ofx::patch::synth::QuickSampler::out_1() {
    return out("1");
}
pdsp::Patchable & ofx::patch::synth::QuickSampler::out_L() {
    return out("0");
}
pdsp::Patchable & ofx::patch::synth::QuickSampler::out_R() {
    return out("1");
}
