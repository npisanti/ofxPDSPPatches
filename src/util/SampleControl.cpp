...
int ofx::patch::util::SampleControl::number = 0;

void ofx::patch::util::SampleControl::patch (){
...
ofParameterGroup & ofx::patch::util::SampleControl::label( std::string name ){
    parameters.setName( name );
    return parameters;
}

void ofx::patch::util::SampleControl::loadButtonCall( bool & value ) {
...
void ofx::patch::util::SampleControl::sampleChangedCall( string & value ) {
...
void ofx::patch::util::SampleControl::loadSample( string path ) {
    sample.load( path );
    if (normalize){
        sample.normalize();
    }
...
void ofx::patch::util::SampleControl::load( string path ) {
    samplePath = path;
}

bool ofx::patch::util::SampleControl::loading() const {
    return loadButton;
}
